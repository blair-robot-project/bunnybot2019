...
public class OmnidirectionalNavXDefaultDrive<T extends Subsystem & DriveOmnidirectional & SubsystemAHRS> extends PIDAngleCommand implements Loggable {
    /**
    ...
    /**
     * Acceleration-limiting ramps for the longitudinal and lateral velocities, respectively. Null for no ramp.
     */
    @Nullable
    private final DoubleUnaryOperator longRamp, latRamp;
    /**
     ...
     */
    @JsonCreator
    public OmnidirectionalNavXDefaultDrive(@JsonProperty(required = true) double absoluteTolerance,
        ...
        this.latRamp = rampComponent;
        this.longRamp = rampComponent != null ? rampComponent.clone() : null; //We want the same settings but different objects, so we clone
    ...
    protected void initialize() {
        ...
        Logger.addEvent("OmnidirectionalNavXArcadeDrive init.", this.getClass());
    ...
    protected void execute() {
        ...
        if (drivingStraight && (oi.getMotionCommandsCached()[2] != 0 || subsystem.getOverrideGyro())) {
        ...
        //If we're free driving and the driver stops turning:
                ...
                oi.getMotionCommandsCached()[2] == 0 && Math.abs(subsystem.getAngularVelCached()) <= maxAngularVelToEnterLoop)) {
        ...
        longOutput = oi.getMotionCommandsCached()[0];
        latOutput = oi.getMotionCommandsCached()[1];

        //Ramp if it exists
        if (longRamp != null) {
            longOutput = longRamp.applyAsDouble(longOutput);
            latOutput = latRamp.applyAsDouble(latOutput);
        }
        ...
        if (drivingStraight) {
            ...
            if (longOutput == 0 && latOutput == 0) {
                finalOutput = deadbandOutput(processedOutput);
            } else {
                finalOutput = processedOutput;
            }

            //Adjust the heading according to the PID output, it'll be positive if we want to go left.
            subsystem.setDirection(longOutput, latOutput, finalOutput);
        }
        //If we're free driving...
            ...
            subsystem.setDirection(longOutput, latOutput, oi.getMotionCommandsCached()[2]);
        }
    }
    ...
    @Override
    protected void end() {
        Logger.addEvent("OmnidirectionalNavXArcadeDrive End.", this.getClass());
    }
    ...
    protected void interrupted() {
        Logger.addEvent("OmnidirectionalNavXArcadeDrive Interrupted! Stopping the robot.", this.getClass());
        subsystem.fullStop();
    }
    ...
    public String getLogName() {
        return "OmnidirectionalNavXDefaultDrive";
    }
}